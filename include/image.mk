...
NTGR_SW_REGION         :=      US
NTGR_HW_ID_NUM         :=      1
NTGR_HW_ID_LIST        :=      "1010000014770000_NETGEAR"
NTGR_MODEL_NUM         :=      4
NTGR_MODEL_LIST        :=      "EXS27;EXS20;EXS25;EXS18"
NTGR_IMAGE_VERSION     :=$(shell cat $(TOPDIR)/package/tw/tw_base-files/files/EXS27/etc/config/system | grep "softVersion" | awk '{print $$3}' | sed "s/'//g")
NTGR_IMAGE_NAME        :=EXS27-$(NTGR_IMAGE_VERSION)\($(REVISION)\).bin
...
define Image/Build/GJEncoder
	cp -rf $(BIN_DIR)/tclinux.bin $(BIN_DIR)/$(1)_unenc
	$(STAGING_DIR_HOST)/bin/imgencoder \
		--encode $(BIN_DIR)/$(1)_unenc \
		--blocksize $(FLASH_NOR_BLOCK) \
		--region $(NTGR_SW_REGION) \
		--model $(1) \
		--sw_version $(NTGR_IMAGE_VERSION) \
		--hwidnum $(NTGR_HW_ID_NUM) \
		--hwidlist $(NTGR_HW_ID_LIST) \
		--modelnum $(NTGR_MODEL_NUM) \
		--modellist $(NTGR_MODEL_LIST) \
		--output $(BIN_DIR)/$(1)_enc
	mv $(BIN_DIR)/$(1)_enc $(BIN_DIR)/$(NTGR_IMAGE_NAME)
	mv $(BIN_DIR)/$(1)_unenc $(BIN_DIR)/$(NTGR_IMAGE_NAME)_unenc
endef
