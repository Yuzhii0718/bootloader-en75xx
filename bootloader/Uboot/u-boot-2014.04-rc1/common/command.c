...
int mtk_mw_cmd(char* mw_addr, char* mw_writeval)
{
	ulong count, default_bytes, bytes, addr, writeval;
	void *buf;
	int	size;

	if(mw_addr == NULL || mw_writeval == NULL)
	...
	/* Translate addr & writeval to ulong in hex */
	addr = simple_strtoul(mw_addr, NULL, 16);
	writeval = simple_strtoul(mw_writeval, NULL, 16);

	/* Get real physical address */
	buf = map_sysmem(addr, default_bytes);
	while (count-- > 0) {
		if (size == 4)
	...
	printf("[MTK[%s][%d] [addr, writeval] = [0x%lx, 0x%lx]\n", __FUNCTION__, __LINE__, addr, writeval);
	unmap_sysmem(buf);
	return 0;
}
