...
#define NMRP_LISTEN_TIMEOUT 5000UL
#endif
...
static int abortboot_normal(int bootdelay)
{
	...
	unsigned long ts;
#if defined(CONFIG_CMD_NMRP)
	int nmrpdelay = ((int)NMRP_LISTEN_TIMEOUT)/1000;

	while (nmrpdelay > bootdelay)
	{
		--nmrpdelay;
		mdelay(1000);
	}
...
#endif
#if defined(CONFIG_CMD_NMRP)
	if (NmrpState != NMRP_STATE_LISTENING && NmrpState != NMRP_STATE_INIT)
	{
		abort = 1;
	}
#endif
