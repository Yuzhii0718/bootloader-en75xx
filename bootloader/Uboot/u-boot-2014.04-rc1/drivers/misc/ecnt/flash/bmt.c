...
*                                                              *
***************************************************************/
void dump_bmt_storedInFlash_info(void)
{
	int i;
	
	printf("BMT v%d.\n", bmt.version);
	printf("total %d mapping\n", bmt.mapped_count);
...
static int migrate_from_bad(int offset, update_reason_t reason, u8 *write_dat, u8 *write_oob)
{
    int page;
...
bool is_beyond_total_block_count(u32 offset)
{
	int block_index = offset / BLOCK_SIZE_BMT;
	return (total_block_count <= block_index) ? true : false;
}

int recover_data_from_bmt(u32 recovered_offset, u32 mapped_offset)
{
	int page;
	u32 mapped_block_index = mapped_offset / BLOCK_SIZE_BMT;
	u32 recovered_block_index = recovered_offset / BLOCK_SIZE_BMT;

	printf("recover_data_from_bmt, block index: recover=%d (0x%x) <-- map=%d (0x%x)\n", 
		recovered_block_index, recovered_offset, mapped_block_index, mapped_offset);

	//Erase block to be recovered.
	if(SPI_NAND_FLASH_RTN_NO_ERROR == spi_nand_erase_block(recovered_block_index))
	{
		//Move data from BMT mapped block to recovered block.
		for(page = 0; page < page_per_block; page++)
		{
			nand_read_page_bmt(PAGE_ADDR(mapped_block_index) + page, dat_buf, oob_buf);

			if (nand_write_page_bmt(PAGE_ADDR(recovered_block_index) + page, dat_buf, oob_buf))
			{
				printf("recover_data_from_bmt, Write page %d fail\n", PAGE_ADDR(recovered_block_index) + page);
				return 1;
			}
		}

		//If move data successfully, erase BMT mapped block.
		if(SPI_NAND_FLASH_RTN_NO_ERROR == spi_nand_erase_block(mapped_block_index))
		{
			return 0;
		}
		else
		{
			printf("recover_data_from_bmt, Erase mapped block %d fail\n", mapped_block_index);
			return 2;
		}
	}
	else
	{
		printf("recover_data_from_bmt, Erase recovered block %d fail\n", recovered_block_index);
		return 3;
	}
}

bool recover_bmt(u32 bad_offset)
{
	int bad_block_index = bad_offset / BLOCK_SIZE_BMT;
	int mapped_offset;
	int i, table_idx = -1;

	printf("recover_bmt, block offset:0x%x (%d), sys(%d), BMT pool(%d)\n", bad_offset, bad_block_index, system_block_count, bmt_block_count);

	//Find the block of bad index and mapped index in BMT table.
	for (i = 0; i < bmt.mapped_count; i++)
    {
        if (bad_block_index == bmt.table[i].bad_index)
            table_idx = i;
    }

	printf("recover_bmt, BMT table[%d]: bad_index=%d, mapped_index=%d\n", table_idx, bmt.table[table_idx].bad_index, bmt.table[table_idx].mapped_index);

	//Delete the mapped information in BMT table and save to flash.
	if(0 <= table_idx)
	{
		//Recover the data.
		mapped_offset = bmt.table[table_idx].mapped_index * BLOCK_SIZE_BMT;
		if(recover_data_from_bmt(bad_offset, mapped_offset))
			return false;
		...
		for(i = 0; i < bmt.mapped_count; i++)
		{
			if(table_idx < i)
			{
				bmt.table[i - 1].mapped_index = bmt.table[i].mapped_index;
				bmt.table[i - 1].bad_index = bmt.table[i].bad_index;
			}
		}

		bmt.table[i].mapped_index = 0;
		bmt.table[i].bad_index = 0;
		bmt.mapped_count--;
	...
	{
		//If the bad block is in the BMT pool, it can be recover (erase) directly.
		//1. Original bad block.
		//2. Became bad block and the original bad block (system) was mapped to the new BMT pool block.
		if((nand_block_bad_bmt(OFFSET(bad_block_index), BAD_BLOCK_RAW) || nand_block_bad_bmt(OFFSET(bad_block_index), BMT_BADBLOCK_GENERATE_LATER)) && system_block_count < bad_block_index)
		{
			if(SPI_NAND_FLASH_RTN_NO_ERROR == spi_nand_erase_block(bad_block_index))
			{
				printf("recover_bmt, erase BMT pool block 0x%x (%d) successfully\n", bad_offset, bad_block_index);
				return true;
			}
			else
			{
				printf("recover_bmt, erase BMT pool block 0x%x (%d) failed\n", bad_offset, bad_block_index);
				return false;
			}
		}
	}
