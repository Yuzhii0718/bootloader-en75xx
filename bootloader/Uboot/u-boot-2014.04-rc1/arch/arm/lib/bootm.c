...
static int __coverity_taint_getenv_check(char *s){
	s = getenv("machid");
	if(s) {
		return 1;
	}
...
static void boot_jump_linux(bootm_headers_t *images, int flag)
{
#ifdef CONFIG_ARM64
#ifdef CONFIG_ECNT
	/* define MACRO are copied from ATF ecnt_sip_svc.h */
	#define ECNT_SIP_KERNEL_BOOT_AARCH32		0x82000200
	#define ECNT_SIP_KERNEL_BOOT_AARCH64		0xC2000200

	struct arm_smccc_res res;
	unsigned int r0 = ECNT_SIP_KERNEL_BOOT_AARCH64;
	unsigned int r1 = images->ep;
	unsigned int r2 = images->ft_addr;
	unsigned int r3 = 0;
	unsigned int r4 = KERNEL_64BITS;
#endif
	...
	if (!fake) {
#if defined(CONFIG_ECNT)
		__arm_smccc_smc(r0, r1, r2, r3, r4, 0, 0 ,0, &res,0);
#else
	...
	//s = getenv("machid");
	__coverity_taint_getenv_check(s);
	if (s) {
