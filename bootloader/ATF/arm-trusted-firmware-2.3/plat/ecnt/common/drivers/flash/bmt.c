...
*                                                              *
***************************************************************/
static void dump_bmt_info(bmt_struct *p_bmt)
{
    int i;

    MSG("BMT v%d.", p_bmt->version);
    MSG("total %d mapping\n", p_bmt->mapped_count);
    for (i = 0; i < p_bmt->mapped_count; i++)
    {
        MSG("%d -> %d \n", p_bmt->table[i].bad_index, p_bmt->table[i].mapped_index);
    }
}
...
static void fill_nand_bmt_buffer(bmt_struct *p_bmt, u8 *dat, u8 *oob)
{
    ...
    phys_bmt->header.mapped_count = p_bmt->mapped_count;
    memcpy(phys_bmt->table, p_bmt->table, sizeof(bmt_entry) * bmt_block_count);
...
static int load_bbt_data(int start, int pool_size, init_bbt_struct *p_bbt)
{
    int i;
    int ret = 0;
    ...
    for(;bbt_index < (start + pool_size); bbt_index++)
    {
        ...
        {
            p_bbt->badblock_count = init_table.header.badblock_count;
            p_bbt->version = init_table.header.version;
            memcpy(p_bbt->badblock_table, init_table.badblock_table, (p_bbt->badblock_count) * 2);

            MSG("bbt found, bad block count: %d\n", p_bbt->badblock_count);

            for (i = 0; i < p_bbt->badblock_count; i++)
            {
                MSG("init_bbt->badblock_table[%d]: %d \n", i, p_bbt->badblock_table[i]);
            }
...
static unsigned short get_bad_index_from_oob(u8 *p_buf)
{
    unsigned short index;

	memcpy(&index, p_buf + oob_bad_index_offset, OOB_INDEX_SIZE);
...
int scan_badblock_raw(init_bbt_struct *p_bbt)
{
    u16 block = 0;
    int count = 0;
    ...
    for(block = 1; block < system_block_count; block++)
    {
        if(nand_block_bad_bmt(OFFSET(block), BAD_BLOCK_RAW))
        {
            if(is_badblock_raw(block))
            {
                ...
                p_bbt->badblock_table[count] = block;
                count++;
            }
            else
            {
...
*******************************************************************/
bmt_struct *reconstruct_bmt(bmt_struct * p_bmt)
{
    int i;
    ...
    p_bmt->version = BMT_VERSION;
    p_bmt->bad_count = 0;
    p_bmt->mapped_count = 0;

    memset(p_bmt->table, 0, bmt_block_count * sizeof(bmt_entry));
    ...
    for (i = 0; i < bmt_block_count; i++, index++)
    {
        ...
        {
            // add mapping to BMT
            p_bmt->table[p_bmt->mapped_count].bad_index = bad_index;
            p_bmt->table[p_bmt->mapped_count].mapped_index = index;
            p_bmt->mapped_count++;
        }
...
init_bbt_struct* reconstruct_bbt(init_bbt_struct* p_bbt)
{
    p_bbt->version = BBT_VERSION;
    p_bbt->badblock_count = 0;
    memset(p_bbt->badblock_table, 0, sizeof(p_bbt->badblock_table));

    if(scan_badblock_raw(p_bbt))
    {
